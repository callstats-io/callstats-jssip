...
function handleSession(session, request, callstats)
{
	...
	{
		mayReportSignalingError('failed', data.cause);
	});

	session.on('ended', (data) =>
	{
		mayReportSignalingError('ended', data.cause);
	});

	function mayReportSignalingError(event, cause)
	{
		...
		switch (cause)
		{
			...
			case causes.AUTHENTICATION_ERROR:
			{
				...
				reportError(callstats.webRTCFunctions.applicationLog, msg);
				break;
			}
			default:
			{
	...
	{
		if (wrtcFuncName === callstats.webRTCFunctions.applicationLog)
